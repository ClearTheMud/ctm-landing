(function () {
  ...
  function init() {
    districtData = window.CTM_DISTRICT_DATA || {};
    ...
    document.addEventListener('click', function (e) {
      if (!e.target.closest('.district') && !e.target.closest('#district-tooltip')) {
        hideTooltip();
      }
    });
  }
  ...
  function getTooltipText(name, slug, isActive) {
    if (!isActive) return name + ' — Research coming soon';
    var info = districtData[slug];
    if (info && info.candidates > 0) {
      var c = info.candidates === 1 ? '1 candidate' : info.candidates + ' candidates';
      return name + ' — ' + c;
    }
  ...
  function setupDistrict(el) {
    ...
    var isActive = slug in districtData;

    el.classList.add(isActive ? 'district--active' : 'district--inactive');
    ...
    el.addEventListener('touchstart', function (e) {
      ...
      var cx = rect.left + rect.width / 2;
      var cy = rect.top;
      if (isActive && slug && tooltip.style.opacity === '1' && tooltip._currentDistrict === slug) {
        navigate(slug);
      } else {
        e.preventDefault();
        showTooltip(tipText, cx, cy);
        positionTooltipAbove(cx, cy);
        tooltip._currentDistrict = slug;
      }
    }, { passive: false });
  }

  function navigate(slug) {
    window.location.href = '/races/' + slug + '/';
  }
